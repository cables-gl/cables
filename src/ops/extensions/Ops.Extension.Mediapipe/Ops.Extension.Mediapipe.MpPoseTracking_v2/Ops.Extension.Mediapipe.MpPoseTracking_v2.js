...
inUpdate.onTriggered = () =>
{
    ...
    if (el.readyState >= 2 && !el.paused && !el.ended)
    {
        pose.send({ "image": el }).then(() => {}).catch((e) =>
        {
...
inUpdateSeg.onTriggered = () =>
{
    ...
    if (!canvasTexture) canvasTexture = new CGL.Texture.createFromImage(cgl, segMaskBitmap);
    else canvasTexture.initTexture(segMaskBitmap, CGL.Texture.FILTER_LINEAR);
...
pose.onResults((r) =>
{
    if (r && r.poseLandmarks)
    {
        ...
        outLandmarks.setRef(r.poseLandmarks);
