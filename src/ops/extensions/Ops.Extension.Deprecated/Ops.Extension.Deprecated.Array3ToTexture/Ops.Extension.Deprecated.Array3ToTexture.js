...
function update()
{
    ...
    let width = Math.ceil(Math.sqrt(arr.length / 3));
    let height = Math.ceil(arr.length / 3 / width);
    ...
    for (j = 0; j < width * height; j++)
    {
        ...
        pixels[idx * 4 + 0] = (arr[idxa * 3 + 0] + 0.0) * 255;
        pixels[idx * 4 + 1] = (arr[idxa * 3 + 1] + 0.0) * 255;
        pixels[idx * 4 + 2] = (arr[idxa * 3 + 2] + 0.0) * 255;
        pixels[idx * 4 + 3] = 255;
    }
