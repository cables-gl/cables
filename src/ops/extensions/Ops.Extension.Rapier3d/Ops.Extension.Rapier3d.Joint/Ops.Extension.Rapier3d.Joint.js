const
    ...
    inType = op.inSwitch("Type", ["Revolute", "Prismatic", "Spherical"], "Revolute"),
...
function setup()
{
    ...
    inLimit.setUiAttribs({ "greyout": inType.get() != "Prismatic" });

    inLimitx.setUiAttribs({ "greyout": inType.get() != "Prismatic" && !inLimit.get() });
    inLimity.setUiAttribs({ "greyout": inType.get() != "Prismatic" && !inLimit.get() });

    op.setUiAttrib({ "extendTitle": inType.get() });
}
