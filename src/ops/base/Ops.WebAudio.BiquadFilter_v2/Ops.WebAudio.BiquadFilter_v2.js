...
inFrequency.onChange = () => {
    ...
    if (freq) {
        ...
        if (freq < FREQUENCY_MIN) {
            op.setUiError("freqRange", "The frequency you selected is lower than the possible range.", 0);
        } else if (freq > FREQUENCY_MAX) {

            op.setUiError("freqRange", "The frequency you selected is higher than the possible range.", 0);
        }
    }
...
inQ.onChange = () => {
    ...
    if (q < Q_MIN) op.setUiError("qRange", "Your Q value is below the minimum possible range of " + Q_MIN + ".", 1);
    else if (q > Q_MAX) op.setUiError("qRange", "Your Q value is above the maximum possible range of " + Q_MAX + ".", 1);
    else {
...
inGain.onChange = () => {
    ...
    if (gain < GAIN_MIN) op.setUiError("gainRange", "Your gain value is below the minimum possible range of " + GAIN_MIN + ".", 1);
    else if (gain > GAIN_MAX) op.setUiError("gainRange", "Your gain value is above the maximum possible range of " + GAIN_MAX + ".", 1);
    else {
