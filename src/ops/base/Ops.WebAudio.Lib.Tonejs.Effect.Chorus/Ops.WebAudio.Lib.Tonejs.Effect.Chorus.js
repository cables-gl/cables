...
var delayTimePort = op.addInPort( new Port( op, "Delay Time", CABLES.OP_PORT_TYPE_, { 'display': 'range', 'min': DELAY_TIME_MIN, 'max': DELAY_TIME_MAX } ));
delayTimePort.set(DELAY_TIME_DEFAULT);
var oscillatorTypePort = op.addInPort( new Port( op, "Oscillator Type", CABLES.OP_PORT_TYPE_, { display: 'dropdown', values: OSCILLATOR_TYPES } ) );
oscillatorTypePort.set(OSCILLATOR_TYPE_DEFAULT);
var spreadPort = op.addInPort( new Port( op, "Spread", CABLES.OP_PORT_TYPE_, { 'display': 'range', 'min': SPREAD_MIN, 'max': SPREAD_MAX } ));
