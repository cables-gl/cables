...
function dorender()
{
    ...
    // shader.setUniformTexture(textureUniform, inTex.get());
    // outTex.set(null);
    ...
    if (resetSpawns)
    {
        // if (inTexSpawns.get()) shader.pushTexture(textureUniform, inTexSpawns.get().tex);

        inTex.set(inTexSpawns.get());

        resetSpawns = false;
        // console.log("resetSpawns");
        // if (inTexSpawns.get())console.log("yes")
    }
    // else
    {
    ...
    if (inTexLifetimes.get()) shader.pushTexture(unitexLifetimes, inTexLifetimes.get().tex);

    const finTex = texMath.render(shader);
    // shader.setUniformTexture(unitexSpawns, inTexSpawns.get().tex);
    // shader.setUniformTexture(unitexLifetimes, inTexLifetimes.get().tex);
