...
const inTexAO = op.inTexture("Occlusion");
const inTexRM = op.inTexture("Roughness/Metallic");
...
op.setPortGroup("Textures", [inTexAlbedo, inTexRM, inTexAO, inTexNormal, inTexEmission, inTexHeight, inLightmap, inTexThinFilm]);
...
function doRender()
{
    ...
    if (inTexAO.get()) PBRShader.pushTexture(inAOUniform, inTexAO.get().tex);
    if (inTexRM.get()) PBRShader.pushTexture(inRMUniform, inTexRM.get().tex);
