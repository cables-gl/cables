...
function remove(mode)
{
    if (ele)
    {
        ...
        ele.style.removeProperty(mode + "-position");
        ele.style.removeProperty(mode + "-repeat");
    }
}
...
function update()
{
    ...
    if (ele && ele.style)
    {
        ...
        if (!active.get())
        {
            if (!inNotUnset.get()) remove(inMode.get());
        }
        else
        {
            let cb = "";
            ...
            else if (ownBackground)
            {
                if (!inNotUnset.get()) ele.style.removeProperty(mode + "-image");
            }
