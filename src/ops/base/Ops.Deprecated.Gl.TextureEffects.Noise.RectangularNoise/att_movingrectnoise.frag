...
float rround(float f,float off)
{
    // return f;
    return floor( (f+ off) * 3.0 + 0.5 ) / 3.0;
...
void main()
{
    ...
    // p=vec2(texCoord.x-0.5,texCoord.y-0.5);
    // p=p*scale;
    // p=vec2(p.x+0.5-x,p.y+0.5-y);

    // p.x=rround(p.x+0.02,1.131+x);
    // p.y=rround(p.y+0.02,2.0131-y);

    // float vv=Perlin3D(vec3(p.x,p.y,z+0.02))*0.5+0.5;
    // v+=floor(vv+0.35)*0.5;
    
    // v=vv;


    // float vvv=Perlin3D(vec3(p.x,p.y,z+0.4))*0.5+0.5;
    // v+=floor(vvv+0.432)*0.16;
