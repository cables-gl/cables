class DracoDecoderClass
{
    constructor()
    {
        ...
        this.config = {
            "wasm": Uint8Array.from(atob(staticAttachments.draco_decoder_wasm), (c) => { return c.charCodeAt(0); }),
            "wrapper": atob(staticAttachments.draco_wasm_wrapper_js),
            "decoderSettings": {},
        };
    ...
    _DracoWorker()
    {
        ...
        let dracoAttributes = function (draco, decoder, dracoGeometry, geometryType, name)
        {
            ...
            let uvCount = 0, uv1Att = null;
            for (let i = 0; i < dracoGeometry.num_attributes(); i++)
            {
                const att = decoder.GetAttribute(dracoGeometry, i);
                if (att.attribute_type() === draco.TEX_COORD)
                {
                    if (uvCount > 0)
                    {
                        const attributeType = attributeTypes.uv;
                        geometry.attributes.push(decodeAttribute(draco, decoder, dracoGeometry, "uv" + uvCount, attributeType, att));
                        break;
                    }
                    uvCount++;
                }
