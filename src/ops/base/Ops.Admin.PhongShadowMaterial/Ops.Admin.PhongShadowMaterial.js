    ...
    if (cgl.frameStore.shadowMap || cgl.frameStore.shadowCubeMap) {
        if (inShadow.get()) {
            ...
            if (cgl.frameStore.shadowMap) {
                cgl.setTexture(6, cgl.frameStore.shadowMap.tex);
                uniformShadowMapSize.setValue(cgl.frameStore.shadowMap.width);
            }

            if (cgl.frameStore.shadowCubeMap) {
                cgl.setTexture(7, cgl.frameStore.shadowCubeMap, cgl.gl.TEXTURE_CUBE_MAP);
...
let shadowMap = new CGL.Uniform(shader,'t','shadowMap', 6);
let shadowCubeMap = new CGL.Uniform(shader, 't', 'shadowCubeMap', 7);
